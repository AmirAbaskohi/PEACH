...
from peach.data import infeed
from peach.params import all_params  # pylint: disable=unused-import
from peach.params import estimator_utils
...
flags.DEFINE_boolean("full", True, "Evaluate full dev dataset.")
...
flags.DEFINE_integer("total_predictions", -1, "number of predictions.")
flags.DEFINE_integer("count_save", 1000000, "number of predictions to save.")
flags.DEFINE_integer("start_point", 0, "start point of dataset.")
...
def main(_):
  ...
  while True:
    if tf.train.checkpoint_exists(FLAGS.model_dir):
      ...
      count = FLAGS.total_predictions
      i = FLAGS.start_point
      count -= i * FLAGS.count_save
      while count > 0:
        step_count = min(FLAGS.count_save,count)
        input_fn = infeed.get_input_fn_predict(params.parser, pattern,
                                        tf.estimator.ModeKeys.PREDICT,skip=i,count=step_count)
        i+=1
        count -= step_count
