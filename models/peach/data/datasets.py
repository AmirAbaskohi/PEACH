...
class TFDSDataset(BaseDataset):
  ...
  def build(self, input_pattern, shuffle_files):
    ...
    Returns:
      Tuple of (tf.data.Dataset, number_of_examples)
    """
    my_train_Dataset_patterns = [input_pattern]
    final_dataset = None
    final_dataset_count = 0
    for dataset in my_train_Dataset_patterns:
      input_pattern = dataset
      ...
      final_dataset_count += num_examples
    return final_dataset, final_dataset_count
