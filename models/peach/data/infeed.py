...
from peach.data import all_datasets
...
def get_input_fn_predict(parser_fn,
                 input_pattern,
                 mode,
                 prefetch=True,
                 drop_remainder=True,
  ...
  def input_fn(params):
    ...
    dataset = dataset.map(parser, num_parallel_calls=parallelism)
    dataset = dataset.unbatch()
    dataset = dataset.skip(skip*count)
...
def get_input_fn_few_shot(parser_fn,
                 input_pattern,
                 mode,
                 prefetch=True,
                 drop_remainder=True,
  ...
  def input_fn(params):
    """Input function."""
    dataset = all_datasets.get_dataset(input_pattern, False)
    dataset = dataset.map(parser, num_parallel_calls=parallelism)
    dataset = dataset.unbatch()
    if training:
      # dataset = dataset.shuffle(10000)
